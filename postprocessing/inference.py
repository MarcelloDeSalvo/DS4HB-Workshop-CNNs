...
class RandomCroppingPrediction:
    ...
    def random_cropping_inference(
        self, input_img, with_brain_mask=False, with_classifier=False
    ):
        """
        ...
        if with_brain_mask:
            predicted_brain_patches, brain_count_map = self.initialize_brain_arrays(
                padded_shape
            )

        if with_classifier:
            predicted_classifier_patches, classifier_count_map = (
                self.initialize_classifier_arrays()
            )
        ...
        for d in range(0, steps_d * self.stride, self.stride):
            for h in range(0, steps_h * self.stride, self.stride):
                for w in range(0, steps_w * self.stride, self.stride):
                    ...
                    predicted_patch, brain_patch, predicted_aux = (
                        self.perform_inference(patch, with_brain_mask, with_classifier)
                    )

                    self.update_prediction_arrays(
                        predicted_patches, count_map, predicted_patch, start, end
                    )

                    if with_brain_mask:
                        self.update_brain_arrays(
                            predicted_brain_patches,
                            brain_count_map,
                            brain_patch,
                            start,
                            end,
                    ...
                    if with_classifier:
                        self.update_classifier_arrays(
                            predicted_classifier_patches,
                            classifier_count_map,
                            predicted_aux,
                        )

        if with_brain_mask:
            final_brain_mask = self.calculate_final_brain_mask(
                predicted_brain_patches, brain_count_map, original_shape
            )
            results["brain_mask"] = final_brain_mask

        if with_classifier:
            final_classifier_output = self.calculate_final_classifier_output(
                predicted_classifier_patches, classifier_count_map
            )
            results["classifier"] = final_classifier_output

        final_prediction = self.calculate_final_prediction(
            predicted_patches, count_map, original_shape
        )
    ...
    def preprocess_input(self, input_img):
        ...
        input_volume = (
            input_img.get_fdata()
            if isinstance(input_img, nib.Nifti1Image)
            else input_img
        )
    ...
    def update_prediction_arrays(
        self, predicted_patches, count_map, predicted_patch, start, end
    ):
        """
        ...
        for i in range(self.num_classes):
            predicted_patches[
                start[0] : end[0], start[1] : end[1], start[2] : end[2], i
            ] += predicted_patch[..., i]
            count_map[start[0] : end[0], start[1] : end[1], start[2] : end[2], i] += 1

    def update_brain_arrays(
        self, predicted_brain_patches, brain_count_map, brain_patch, start, end
    ):
        """
    ...
    def update_classifier_arrays(
        self, predicted_classifier_patches, count_map, predicted_patch
    ):
        """
    ...
    def calculate_steps(self, padded_shape):
        ...
        if steps_d * self.stride < depth:
            steps_d += 1
        if steps_h * self.stride < height:
            steps_h += 1
        if steps_w * self.stride < width:
            steps_w += 1
    ...
    def calculate_patch_bounds(self, d, h, w, padded_shape):
        ...
        return (ranges_d[0], ranges_h[0], ranges_w[0]), (
            ranges_d[1],
            ranges_h[1],
            ranges_w[1],
        )
    ...
    def calculate_final_prediction(self, predicted_patches, count_map, original_shape):
        ...
        if self.num_classes == 1:
            final_prediction = (predicted_patches >= self.threshold).astype(np.float64)[
                ..., 0
            ]
        else:
    ...
    def calculate_final_brain_mask(
        self, predicted_brain_patches, brain_count_map, original_shape
    ):
        """
        ...
        final_brain_mask = (predicted_brain_patches >= self.threshold).astype(
            np.float64
        )[..., 0]
        return final_brain_mask[
            : original_shape[0], : original_shape[1], : original_shape[2]
        ]

    def calculate_final_classifier_output(
        self, predicted_classifier_patches, count_map
    ):
        """
...
class FullVolumeEvaluation:
    ...
    def __init__(
        ...
        strides,
        preprocessor,
        brain_mask_postprocess_callback=None,
        roi_postprocess_callback=None,
        path=None,
        verbose=False,
        threshold=0.5,
    ):
        ...
        self.logger = (
            PrintLogger(
                os.path.join(self.results_path, "evaluation.log"), verbose=verbose
            )
            if self.results_path is not None
            else None
        )
        self.metrics = [
            dice_coefficient(average="micro"),
            precision_coefficient(average="micro"),
            specificity_coefficient(average="micro"),
            sensitivity_coefficient(average="micro"),
            iou_coefficient(average="micro"),
            volume_similarity_coefficient(average="micro"),
        ]
    ...
    def evaluate(self, with_brain_mask=True, evaluate_brain_mask=False):
        ...
        croppingInference = RandomCroppingPrediction(
            self.model, self.patch_size, self.num_classes, self.strides, self.threshold
        )
        ...
        for stride in self.strides:
            ...
            dataset_metrics = self.evaluate_dataset(
                stride, unique_modality_test_ids, croppingInference
            )
        ...
        if self.results_path is not None:
            stride_metrics.to_excel(
                os.path.join(self.results_path, "evaluation_results.xlsx")
            )

        # Plot all metrics
        final_df = self.plot_all_metrics(
            stride_metrics,
            os.path.join(self.results_path, "metrics_per_stride_and_modality.png"),
        )
        self.logger.close()
        return final_df
    ...
    def evaluate_dataset(self, stride, unique_modality_test_ids, croppingInference):
        ...
        for mod_idx, subjects in enumerate(unique_modality_test_ids):
            ...
            for key in subjects:
                ...
                subject_metrics_dataframe = self.compute_metrics_for_subject(
                    subject, croppingInference
                )
            ...
            for metric in self.metrics:
                ...
                self.plot_subjects_metric_per_class(
                    sub_series,
                    metric.__name__,
                    f"_stride_{stride}_modality_{cur_modality}",
                )
            ...
            subjects_metrics = subjects_metrics[
                ["Subject", "Stride", "Modality"]
                + [
                    col
                    for col in subjects_metrics.columns
                    if col not in ["Subject", "Stride", "Modality"]
                ]
            ]
    ...
    def compute_metrics_for_subject(self, subject, croppingInference):
        ...
        if self.evaluate_brain_mask:
            ...
            processed_image, [preprocessed_gt, preprocessed_mask] = (
                self.preprocess_data(image, [seg_gt, mask_gt])
            )
        else:
            image, seg_gt, _ = self.load_data(subject)
            processed_image, preprocessed_gt = self.preprocess_data(image, seg_gt)

        results = croppingInference.random_cropping_inference(
            processed_image, self.with_brain_mask
        )
        ...
        if self.num_classes > 1:
            final_prediction = tf.keras.utils.to_categorical(
                final_prediction, self.num_classes
            )
        else:
        ...
        class_df = self.compute_metrics_per_class(
            prep_gt_data, final_prediction, subject.get_id()
        )
        return class_df

    def compute_metrics_per_class(
        self, preprocessed_ground_truth, final_prediction, id
    ):
        """
        ...
        for class_idx in range(self.num_classes):
            ...
            metrics = {
                metric.__name__: metric(class_gt, class_pred).numpy()
                for metric in self.metrics
            }
    ...
    def get_unique_test_ids(self):
        ...
        for index, modality in enumerate(self.modalities):
            self.logger.write(
                f"Unique test ids for modality {modality}:\n - {unique_modality_test_ids[index]}"
            )
    ...
    def plot_subjects_metric_per_class(self, subjects_df, metric, file_name):
        ...
        sns.pointplot(
            y=metric, x="Subject", data=df, ax=axes[0], palette="deep", hue="Class"
        )
        ...
        # Plot box plot
        sns.boxplot(
            y=metric, hue="Class", data=df, ax=axes[1], palette="deep", legend=False
        )
        ...
        if self.results_path is not None:
            ...
            subjects_df.to_excel(
                os.path.join(save_path, metric + "_" + file_name + ".xlsx")
            )
    ...
    def plot_all_metrics(self, all_metrics_data, output_file):
        ...
        all_metrics_data.to_excel(
            os.path.join(self.results_path, "all_metrics_data.xlsx")
        )
    ...
    def plot_results(self, stride_average_dices_df, metric, ax):
        ...
        stride_average_dices_df["Mean"] = stride_average_dices_df.iloc[
            :, first_class_index:
        ].mean(axis=1)
        ...
        melted_df = pd.melt(
            stride_average_dices_df,
            id_vars=["Subject", "Stride", "Modality", "Metric"],
            value_vars=["Mean"],
            var_name="Class",
            value_name="Means",
        )
        ...
        sns.boxplot(
            x="Stride", y="Means", hue="Modality", data=melted_df, ax=ax, palette="deep"
        )
        ...
        ax.legend(
            handles[: len(set(labels))], labels[: len(set(labels))], fontsize="small"
        )
