...
def plot_2d_slices(
    ...
    if slice_idx is None:
        slice_idx = processed_image.shape[axis] // 2
    if slice_idx < 0 or slice_idx >= processed_image.shape[axis]:
        raise ValueError(
            f"slice_idx must be between 0 and {processed_image.shape[axis]-1}"
        )
    ...
    plt.subplot(1, 5, 1)
    plt.imshow(
        np.rot90(np.take(processed_image, slice_idx, axis=axis), k=rotation),
        cmap="gray",
    )
    ...
    plt.subplot(1, 5, 2)
    plt.imshow(
        np.rot90(np.take(ground_truth, slice_idx, axis=axis), k=rotation), cmap=cmap
    )
    plt.title("Ground Truth")

    plt.subplot(1, 5, 3)
    plt.imshow(
        np.rot90(np.take(prediction, slice_idx, axis=axis), k=rotation), cmap=cmap
    )
    ...
    plt.imshow(
        np.rot90(np.take(prediction, slice_idx, axis=axis), k=rotation),
        alpha=alpha,
        cmap=cmap,
    )
    plt.title("Overlap")
    ...
    plt.subplot(1, 5, 5)
    plt.imshow(
        np.rot90(np.take(difference, slice_idx, axis=axis), k=rotation), cmap="jet"
    )
...
def plot_3d_overlap(
    ...
    sample_img = np.swapaxes(sample_img, 0, 2)
    sample_mask = np.swapaxes(sample_mask, 0, 2)

    # Invert Z-axis (just for visual purposes)
    sample_img = sample_img[::-1, ::-1, ::-1]
    sample_mask = sample_mask[::-1, ::-1, ::-1]

    # Brain and Gt
    _sample_mask = np.where(sample_mask > 0.5, 1, 0)
    gt_indices = np.argwhere(_sample_mask > 0)
    brain_indices = np.argwhere(sample_img > noise_treshold)  # remove noise

    # Randomly sample points based on the specified density
    brain_indices = brain_indices[
        np.random.choice(len(brain_indices), brain_density, replace=False)
    ]
    gt_indices = gt_indices[
        np.random.choice(len(gt_indices), mask_density, replace=False)
    ]

    # Get matrix values for color mapping
    brain_colors = sample_img[
        brain_indices[:, 0], brain_indices[:, 1], brain_indices[:, 2]
    ]
    ...
    fig.update_layout(
        ...
        title="3D Plot of Brain and Segmentation Mask",
    )
